...
# -----------------------------------------------------------------------------
GRPC_SWIFT_VERSION = 2.0.0-alpha1.1
PROTOC_GRPC_SWIFT_PLUGINS_SHA256SUM = f0b906fc11e72086f333fb164a2442b15d24370769da4583e332b202abb0f1b3
PROTOC_GRPC_SWIFT_PLUGINS_DOWNLOAD_CACHE_DIR = .protoc-grpc-swift-plugins.download
PROTOC_GRPC_SWIFT_PLUGINS_URL = https://github.com/grpc/grpc-swift/archive/refs/tags/2.0.0-alpha.1.zip
...
$(PROTOC_GRPC_SWIFT_PLUGINS_ZIP):
	mkdir -p $(PROTOC_GRPC_SWIFT_PLUGINS_DOWNLOAD_CACHE_DIR)
	cp /usr/local/bin/protoc-pluginsV2/protoc-plugins.zip $@
...
$(OTLP_CORE_SWIFTS): $(OTLP_CORE_PROTOS) $(PROTO_MODULEMAP) $(PROTOC_GEN_SWIFT)
	...
	PATH=$(PATH):$(PROTOC_GRPC_SWIFT_PLUGINS_ROOT)/bin protoc $(OTLP_CORE_PROTOS) \
...
$(OTLP_CLIENT_GRPC_SWIFTS): $(OTLP_GRPC_PROTOS) $(PROTO_MODULEMAP) $(PROTOC_GEN_GRPC_SWIFT)
	...
	PATH=$(PATH):$(PROTOC_GRPC_SWIFT_PLUGINS_ROOT)/bin protoc $(OTLP_GRPC_PROTOS) \
		--proto_path=$(PROTO_ROOT) \
